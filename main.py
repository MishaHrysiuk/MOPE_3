...
x3_min = 10
x3_max = 20
xc_max = (x1_max + x2_max + x3_max)/3.0
xc_min = (x1_min + x2_min + x3_min)/3.0
y_min = 200 + xc_min
y_max = 200 + xc_max
m = 3 # кількість дослідів
n = 4 # кількість експерментів
q = 0.5
...
def main(m, n, q):
    ...
    if(gP < gT):
        print(f"Варіантність однорідна: Gp = {gP:.5} < Gt = {gT}")
    else:
        print(f"Варіантність не однорідна Gp = {gP:.5} < Gt = {gT}")
        m = m + 1
        main(m + 1, n, q)
        return
    ...
    print("Нормовані зхначення: ", ckeck_list)
